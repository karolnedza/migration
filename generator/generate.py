...
def create_excel_headers(worksheet):
    ...
    worksheet.write(0, 3, "aws_region")
    worksheet.write(0, 4, "vpcs")
    ...
    worksheet.write(0, 13, "insane_mode")
    worksheet.write(0, 14, "insane_az1")
    worksheet.write(0, 15, "insane_az2")
    ...
    for account in accounts:
        ...
        for region in regions:
            ...
            for vpc in vpcs:
                ...
                worksheet.write(row, 13, defaults["insane_mode"])

                insane_subnets = get_available_subnets(
                    get_subnets(vpc, ec2), vpc["CidrBlock"], 26
                )
                try:
                    insane_az1 = (
                        '["' + insane_subnets[0] + '","' + defaults["insane_az1"] + '"]'
                    )
                except IndexError:
                    insane_az1 = '["","' + defaults["insane_az1"] + '"]'
                try:
                    insane_az2 = (
                        '["' + insane_subnets[1] + '","' + defaults["insane_az2"] + '"]'
                    )
                except IndexError:
                    insane_az2 = '["","' + defaults["insane_az2"] + '"]'
                worksheet.write(row, 14, insane_az1)
                worksheet.write(row, 15, insane_az2)
